 ...
 *
 * ── Why this exists ────────────────────────────────────────────────────────
 * The store was once wired to exactly one gateway. When that provider deactivated
 * the account, checkout did not degrade — it stopped. A single provider is a single
 ...
 *     created with — never on whatever is preferred today. That is what keeps an
 *     existing order verifiable and refundable after the default has moved on,
 *     which is a hard requirement: they are GST records.
 *
 * ── Money units ────────────────────────────────────────────────────────────
 * A live bug waiting to happen, so it is stated once here: both gateways price in
 * integer PAISE, while every Nemo order is priced in RUPEES. Each adapter converts
 * at its own boundary and every function in this module that crosses the boundary
 * says which unit it means in its name or its comment. Nothing outside an adapter
 * should ever see paise.
 *
 * ── Configuration ──────────────────────────────────────────────────────────
 ...
 *
 *   PAYMENT_PROVIDER_ORDER    optional ops-level override; if unset, the order comes from
 *                             the admin's "Primary gateway" choice in Settings instead
 *                             (settings.paymentPrimary — switchable with no deploy)
 *
 ...
 *
 * Order comes from, in priority: an explicit PAYMENT_PROVIDER_ORDER env var (an ops-level
 * override, set once and not meant for routine use); otherwise the admin's "Primary gateway"
 * choice in Settings (settings.paymentPrimary), which the storefront's checkout panel reads
 * live with no deploy needed; otherwise PhonePe first. Whichever is first is tried first —
 * the other is still offered as automatic failover if the first can't create a session.
 */
export async function availableProviders() {
  const envOrder = String(process.env.PAYMENT_PROVIDER_ORDER || '').trim();
  let configured;
  if (envOrder) {
    configured = envOrder.split(',').map(s => s.trim().toLowerCase()).filter(Boolean);
  } else {
    let primary = '';
    try { primary = String((await dbGet('settings'))?.paymentPrimary || '').trim().toLowerCase(); }
    catch { /* no settings yet, or the database is unreachable — fall back to the default below */ }
    configured = primary === 'razorpay' ? ['razorpay', 'phonepe'] : ['phonepe', 'razorpay'];
  }
...
export async function finalizePayment(providerId, userUid, orderId, gatewayOrderId) {
  ...
  /* A refunded order must never be walked back to "paid".
     Neither gateway retires the underlying payment when money is sent back — a PhonePe order
     stays COMPLETED after a refund, and a Razorpay payment stays `captured` after a partial
     one. So a refund webhook (or any later re-verification) still passes every check in
     `confirm` above, and would otherwise overwrite the refund with a fresh Verified/Confirmed.
     The payment facts below stay accurate and are still written; only the status fields that
     the refund owns are withheld. */
  const refunded = money(order.refundedAmount || 0) > 0;
  const patch = {
    gateway: provider.id,
...
export const paymentsReady = async () => (await availableProviders()).length > 0 && firebaseReady();
 ...
 */
export const allProvidersSandbox = async () => {
