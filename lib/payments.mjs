 ...
 * Nemo order. It intentionally moves the order to Payment Review instead of
 * fulfilment: the owner still accepts the order and verifies its catalogue price,
 * stock, and any discount before marking it Confirmed.
 *
...
export async function finalizeCashfreePayment(userUid, orderId, cashfreeOrderId) {
  ...
  const patch = {
    ...
    paymentStatus: sandbox ? 'Test Paid' : 'Gateway Paid — Review',
