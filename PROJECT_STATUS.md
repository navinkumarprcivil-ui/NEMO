...
**Still open — the GST discount bug (deliberately NOT changed yet, awaiting the owner's CA).** `generateInvoiceHTML` and `orderGST` compute taxable value on the **pre-discount** gross and subtract coupon/referral/wallet afterwards, so output tax is over-declared on every discounted order: ₹15.25 on a ₹1,100 order with a ₹100 coupon, ₹38.14 on a ₹5,250 order with ₹250 of discounts (verified numerically). §15(3)(a) allows a discount shown on the invoice at the time of supply to be excluded from taxable value. Related: **shipping is always taxed at the default rate** (booked to HSN 9968) even when the goods are nil-rated — for a composite supply freight should follow the principal supply's rate.
