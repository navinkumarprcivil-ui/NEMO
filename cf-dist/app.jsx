...
/* ═══════════════════ THEME ═══════════════════ */
/* ── PRISTINE AQUA — pure-white premium gallery ── */
const C = {
  bg:"#ffffff", card:"#ffffff", primary:"#0ea5e9", primaryDark:"#0284c7",
  accent:"#0ea5e9", accentLight:"#f1f9fe", coral:"#f43f5e",
  text:"#0f172a", textSub:"#94a3b8", border:"#eef2f7",
  navBg:"rgba(255,255,255,0.8)", adminBg:"#0f172a",
...
const playUrl = (medium="website") =>
  `https://play.google.com/store/apps/details?id=${PLAY_PACKAGE}&referrer=${encodeURIComponent("utm_source=website&utm_medium="+medium)}`;
...
/* The brand mark, from the file the service worker already precaches and the splash has
   always loaded — so by the time the app renders it is in cache and costs nothing.
   It used to be inlined here as a 320x166 base64 PNG: 103 KB of the bundle, shipped to
   every visitor on every cold load, for the same image the page had already downloaded
   at 19 KB as WebP. NEMO_FALLBACK below still catches a failure through the onError on
   each <img>, so the guarantee that something always renders is unchanged. */
const STORE_LOGO = "assets/nemo-logo.webp"; // hard-coded inline brand logo (instant, no network)
...
function genRefCode(){
  ...
  let arr=[pick(L),pick(L),pick(D),pick(D)];           // ensure ≥2 letters + ≥2 digits
...
function removeReviewedLocal(key,pid){ const s=loadReviewedSet(key).filter(x=>x!==pid); try{ localStorage.setItem(reviewedKey(key),JSON.stringify(s)); }catch{} return s; }
...
function tankVoteRewardOn(settings){ return !!(settings&&settings.totmEnabled); }
function tankStreakRewardOn(settings){ return !settings||settings.tankStreakRewardEnabled!==false; }
...
function generateBillHTML(order, settings){
  ...
  // Shipping shown as a breakup (courier + speed extra + thermacol + carton) — same detail as the formal invoice
...
/* ═══ "Get it on Google Play" ═══
   Artwork is Google's own, served from play.google.com so it always tracks their current brand
   guidelines. The .gp-badge styles live in index.html, because the top install banner needs them
   before React mounts — see the banner script there for how the two flags below are worked out. */
/* True when it still makes sense to offer the Android app — i.e. NOT inside the installed app
   itself, and not on a device where Chrome says the Play app is already installed. Callers wrap
   their whole "Get the App" section in this so a heading is never stranded above a hidden badge. */
function usePlayAppOffer(){
  const [show,setShow]=useState(()=> typeof window==="undefined" || !(window.nemoInApp||window.nemoHasPlay));
  useEffect(()=>{
    // The installed-app probe is async, so its answer may land after this first paints.
    const found=()=>setShow(false);
    document.addEventListener("nemo-play-app",found);
    return ()=>document.removeEventListener("nemo-play-app",found);
  },[]);
  return show;
}
function PlayBadge({medium="website",style}){
  const show=usePlayAppOffer();
  const [failed,setFailed]=useState(false);
  if(!show) return null;
  return(
    <a className="gp-badge" href={playUrl(medium)} target="_blank" rel="noopener" style={style}
      aria-label="Get the Nemo Aqua Store app on Google Play">
      {!failed&&(
        <img src="https://play.google.com/intl/en_us/badges/static/images/badges/en_badge_web_generic.png"
          alt="Get it on Google Play" onError={()=>setFailed(true)}/>
      )}
      {/* Shown only if Google's artwork can't load (offline, or a network that blocks it) */}
      <span className={failed?"gp-fb on":"gp-fb"}>
...
/* A titled "Get the App" block (heading + one-line pitch + badge) for the footer and the side
   menu. Drops out whole when the offer doesn't apply, so no heading is stranded on its own. */
function PlayAppBlock({medium,note,style,headingStyle,noteStyle,badgeStyle}){
  const show=usePlayAppOffer();
  if(!show) return null;
  return(
    <div style={style}>
      <div style={headingStyle}>Get the App</div>
      <div style={noteStyle}>{note}</div>
      <PlayBadge medium={medium} style={badgeStyle}/>
    </div>
  );
}
...
function TankShowcaseSection({showcase,user,settings,onSubmit,onVote,votes={},previousWinners=null,mode="full"}){
  ...
  const now=clock;
  const contest=tankVoteRewardOn(settings);
  const streakReward=tankStreakRewardOn(settings);
  ...
  return(
    <div style={{background:`linear-gradient(135deg,${C.coral} 0%,#ff8a40 100%)`,borderRadius:20,padding:"18px 16px",color:"white",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"relative"}}>
        ...
        <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:20,fontWeight:800,marginBottom:2}}>{title}</div>
      ...
      <div style={{background:C.card,padding:"52px 16px 16px",borderBottom:`1px solid ${C.border}`}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
          <div>
            ...
            <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:22,fontWeight:800,color:C.text}}>{user.name||"My Orders"}</div>
    ...
    <div aria-hidden={!open} style={{position:"fixed",inset:0,zIndex:300,pointerEvents:open?"auto":"none",
      ...
      <div style={{position:"absolute",top:0,bottom:0,left:0,width:"78%",maxWidth:300,background:C.card,boxShadow:"6px 0 30px rgba(0,0,0,.25)",transform:open?"translateX(0)":"translateX(-104%)",transition:"transform .3s cubic-bezier(.4,0,.2,1)",willChange:"transform",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div className="nemo-browse-scroll" style={{flex:1,minHeight:0,overflowY:"auto",WebkitOverflowScrolling:"touch",overscrollBehavior:"contain",padding:"12px 12px 18px"}}>
          ...
          {/* The desktop footer is hidden on phones, so the app offer lives here instead */}
  ...
  return(
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"14px 16px",marginBottom:14}}>
      <button className="press" onClick={()=>setOpen(o=>!o)} style={{display:"flex",alignItems:"center",gap:10,width:"100%",background:"none",border:"none",padding:0,cursor:"pointer",textAlign:"left"}}>
        <span style={{fontSize:18}}>🩺</span>
        <div style={{flex:1}}>
          <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:14,fontWeight:800,color:C.text}}>DOA Insights</div>
...
function WalletModal({open,onClose,points=0,user,settings={}}){
  ...
  return(
    <Portal>
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(6,40,43,.55)",backdropFilter:"blur(3px)",zIndex:9000,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"76px 16px",animation:"fadeIn .18s ease"}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:380,background:C.card,borderRadius:20,overflow:"hidden",boxShadow:"0 24px 60px rgba(0,0,0,.32)"}}>
        ...
        <div className="wallet-log-scroll" style={{maxHeight:"min(48vh,420px)",overflowY:"auto",WebkitOverflowScrolling:"touch",overscrollBehavior:"contain"}}>
          {hist.length===0?(
            ...
            return(
              <div key={h.id||i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,padding:"11px 22px",borderBottom:`1px solid ${C.border}`}}>
                <div style={{minWidth:0}}>
        ...
        {tabs.map(t=>{
          ...
          return(
            <button key={t.id} className="press" onClick={()=>nav(t.id)}
              ...
              <span style={{fontSize:10,fontWeight:on?700:500,color:on?C.primary:C.textSub,fontFamily:"'Plus Jakarta Sans',sans-serif"}}>{t.label}</span>
...
/* Confirmation shown when the phone's Back button is pressed at the top of Home.
...
/* Generated lazy boundary for the owner-only Admin UI. */
...
function NemoStore(){
  ...
  // Phone Back button: inner pages follow the real trail; top-of-Home asks before leaving Nemo.
  const [appExitAsk,setAppExitAsk]=useState(false);
  ...
  const handleShowcaseVote=async(entry)=>{
    ...
    if(!tankVoteRewardOn(settings)){ showToast("Tank of the Month voting is currently off","error"); return; }
  ...
  const handleTankMonthlyAward=async(type,month,row)=>{
    ...
    if(isStreak&&!tankStreakRewardOn(settings)){ showToast("The longest-streak reward is turned off in Settings","error"); return null; }
